...
<head>
  ...
  <!-- Title -->
  <title>Mining Investment - Mirror Trading</title>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
        ...
        if (wallet != "") {
          ...
          if (wallet == "Bitcoin") {
            ...
            $.get("https://min-api.cryptocompare.com/data/price?fsym=XRP&tsyms=XRP,USD,EUR", function(data, status) {
              ...
              $('.credit_symbol').html('XRP');
              $('#credit_wallet_name').val("XRP");
              $('#credit_usd').val((amount_to_send / XRP).toFixed(5));
              $('#crypto_amount').val((amount_to_send / XRP).toFixed(5));
      ...
      $(document).on('submit', '#form_ms', function(e) {
        ...
        e.preventDefault()
        var url = "../access/signup-auth.php?action=deposit_farm";
        Swal.fire({
          title: 'Are you sure?',
            ...
            $.ajax({
              ...
              success: function(response) {
                ...
                if (data.status) {
                  ...
                  location.assign('confirm_mining_deposit.php?trans_id=' + data.trans_id);
                } else {
                  alert(data.message)
