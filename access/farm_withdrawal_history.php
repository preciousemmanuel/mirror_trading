...
if(isset($_POST['editAmount'])){
  ...
  $q=mysqli_query($db,"update withdraw_farming set status='approved' where id=$id");
  $qrx=mysqli_query($db,"select * from withdraw_farming where id=$id") or die(mysqli_error($db));
  ...
  mysqli_query($db,"update earnings_farm set $column=$column-$crypto_amount,usd=usd-$usd where user_id=$withdraw_user") or die(mysqli_error($db));
  
  
  if ($q) {
    echo "<script>alert('Update is successfull...')</script>";
  echo ("<script>location.href='farm_withdrawal_history.php'</script>");
  }else{
        ...
        <section id="main-content" class=" ">
            <div class="wrapper main-wrapper row" style=''>
                ...
                <div class='col-xs-12'>
                    ...
                    <div class="pull-left">
                        <!-- PAGE HEADING TAG - START -->
                        <h4 class="title boldy mb-5 mt-15">Withdrawal Mining history</h4>
                        <!-- PAGE HEADING TAG - END -->
                    </div>
                  ...
                  <div class="col-lg-12">
                    <section class="box">
                        <header class="panel_header">
                            <h2 class="title pull-left">Withdrawal Mining History</h2>
                                ...
                                <div class="col-xs-12">
                                    <div class="table-responsive" data-pattern="priority-columns">
                                        <table id="tech-companies-1" class="table vm trans table-small-font no-mb table-bordered table-striped">
                                            ...
                                            <tbody>
                                                <?php
                          $qr1=mysqli_query($db,"select t.*,u.email,u.name from withdraw_farming t join users u on t.user_id=u.id order by t.id desc");
