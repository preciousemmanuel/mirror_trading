...
if(isset($_POST['editAmount'])){
  ...
  $q=mysqli_query($db,"update earnings set usd=$usd, bitcoin='$bitcoin',etherium='$etherium',litecoin='$litecoin',dogecoin='$dogecoin',bch='$bch' where id=$id");
...
<head>
    ...
    <title>Admin- MIRRORTRADING</title>
            ...
            <div class="wrapper main-wrapper row" style=''>
                ...
                <div class='col-xs-12'>
                    ...
                    <div class="pull-left">
                        ...
                        <h4 class="title boldy mb-5 mt-15">Users and Earnings</h4>
                  ...
                  <div class="col-lg-12">
                    <section class="box">
                        <header class="panel_header">
                            <h2 class="title pull-left">Earnings</h2>
                        ...
                        <div class="content-body">
                                ...
                                <div class="col-xs-12">
                                    <div class="table-responsive" data-pattern="priority-columns">
                                        <table id="tech-companies-1" class="table vm trans table-small-font no-mb table-bordered table-striped">
                                            ...
                                            <tbody>
                                                <?php
                          $qr1=mysqli_query($db,"select t.*, accounts.plan as plan,u.email,u.created_at,u.id as user_id,u.name from users u left join earnings t on t.user_id=u.id left join accounts on accounts.user_id=u.id");
                          ...
                          <td><?= $row['email'] ?></td>
                          <td>
                            <button class="btn btn-primary " data-toggle="modal" data-target="#ref<?= $row['user_id'] ?>">
                              <?php
                                $countX=mysqli_query($db,"select count(*) as count from  users where referal=".$row['user_id']);
                                echo mysqli_fetch_assoc($countX)['count'];
                             ?>
                            </button>

                           <div style="background-color: rgba(0, 0, 0, 0.5);z-index: 1" class="modal fade" id="ref<?= $row['user_id'] ?>" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
          aria-hidden="true">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel" style="color: #000">View Referals</h5>
              ...
              <div class="table">
                <table class="table">
                  ...
                  <tbody>
                    ...
                    $qrx1=mysqli_query($db,"select * from  users where referal=".$row['user_id']) ;
                    if ($qrx1) {
                      while ($rowR=mysqli_fetch_array($qrx1)) {
                            ?>
                            <tr>
                             <td><?= $rowR['name'] ?></td> 
                             <td><?= $rowR['email'] ?></td> 
                             <td>
                              <?php
                              if ($rowR['referal_clearal']=="yes") {
                                echo "Approved";
                              } else {
                                ?>
                              
                              <button type="button"  class="btn btn-success approve_referee" id="<?= $row['user_id'] ?>" data-refx="<?= $rowR['id'] ?>">Approve</button>
                            <?php } ?>
              ...
              <div class="modal-footer bg-whitesmoke br">
                ...
                <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                          ...
                          <td><button title="delete user" class="btn-danger" data-toggle="modal" data-target="#delete<?= $row['id'] ?>"><span class="fa fa-close"></span></button></td>
